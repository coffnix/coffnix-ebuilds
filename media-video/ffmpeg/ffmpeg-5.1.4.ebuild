...
FFMPEG_SUBSLOT="59.37.100"  # ou equivalente da tua libavcodec atual
...
RDEPEND="
	...
	video_cards_nvidia? ( >=media-libs/nv-codec-headers-10.0.26.1[${MULTILIB_USEDEP}] )
	...
	if multilib_is_native_abi; then
		...
		if use chromium; then
			if build_separate_libffmpeg; then
				...
				# it's own so this QA check raises a false positive.
