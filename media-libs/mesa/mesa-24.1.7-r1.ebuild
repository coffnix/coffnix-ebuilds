...
REQUIRED_USE="
    ...
    vdpau? ( !video_cards_panfrost )
    vulkan? ( !video_cards_panfrost )
    xa? ( X )
"
...
pkg_pretend() {
    if use video_cards_panfrost; then
        if use vdpau; then
            ewarn "Disabling USE=vdpau: Panfrost does not support it."
        fi
        if use vulkan; then
            ewarn "Disabling USE=vulkan: Panfrost does not use Vulkan in this build."
        fi
    fi
    if use vulkan; then
...
multilib_src_configure() {
    ...
    if use vulkan; then
        ...
        emesonargs+=(-Dvulkan-layers=)
    fi

    if use video_cards_panfrost; then
        emesonargs+=(-Dglslang=disabled)
    fi
