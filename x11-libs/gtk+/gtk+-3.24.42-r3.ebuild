...
src_prepare() {
	...
	# Aplica patch para evitar instalação de gtk-update-icon-cache pelo gtk+
	# Descomente se você tiver o patch disponível em ${FILESDIR}
...
src_install() {
	...
	# Instala o arquivo de configuração
	insinto /etc/gtk-3.0
	doins "${FILESDIR}"/settings.ini

	# Instala apenas os documentos disponíveis
	DOCS=( NEWS README.md )
	einstalldocs

	# Remove arquivos que pertencem a dev-util/gtk-update-icon-cache (se não usar o patch)
	rm -f "${D}/usr/bin/gtk-update-icon-cache" || die "Failed to remove gtk-update-icon-cache"
	rm -f "${D}/usr/share/man/man1/gtk-update-icon-cache.1.bz2" || die "Failed to remove man page"
}

pkg_postinst() {
	# Atualiza o cache de módulos de entrada diretamente
	if [ -x /usr/bin/gtk-query-immodules-3.0 ]; then
		/usr/bin/gtk-query-immodules-3.0 --update-cache || die "Failed to update immodules cache"
	else
		ewarn "gtk-query-immodules-3.0 not found, skipping immodules cache update"
	fi
